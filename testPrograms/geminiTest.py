...
async def get_entities_example():
    # read data from data2.txt
    text = ""
    with open('data2.txt', 'r') as file:
        text = file.read()

    entities_prompt = GetEntityExtractor(text, "jobs").getText()

    command = ExecutePromptCommand(prompt=entities_prompt, context={})
    result = await gemini_service_provider.execute_prompt(command)
    print("Entities:")
    print(result.content)
